...
function getSesion() {
  ...
  const espacio = db.spaces.find((s) => s.session === true);
...
function cargarUsuariosSelect() {
  ...
  select.innerHTML = espacio.members
    .map((userId) => {
      const user = db.users.find((u) => u.id === userId);
...
function getUserPoints(userId, tasks) {
  const pointsMap = {
    low: 5,
    medium: 10,
    high: 15
  ...
  return tasks
    ...
    .reduce((total, task) => total + (pointsMap[task.priority] || 0), 0);
}

function cargarRanking() {
  const { usuario, espacio, db } = getSesion();

  const spaceUsersIds = espacio.members;

  const spaceUsers = db.users
    .filter(u => spaceUsersIds.includes(u.id))
    .map(user => {
      const points = getUserPoints(user.id, db.tasks);
    ...
    .sort((a, b) => b.points - a.points); // 🔥 ordenar ranking

  const container = document.querySelector(".ranking-all-user");
  if (!container) return;

  container.innerHTML = spaceUsers.map((user, index) => `
...
function cargarEspacio() {
  ...
  const spaceTask = db.tasks.filter((t) => t.spaceId === espacio.id);
  const userTasks = spaceTask.filter((ut) => ut.assignedTo === usuario.id);
  const userTaskDone = userTasks.filter((utd) => utd.status === "done");

  const userPoints = userTaskDone.reduce((total, task) => {
    let points = 0;

    if (task.priority === "low") points = 5;
    else if (task.priority === "medium") points = 10;
    else if (task.priority === "high") points = 15;

    return total + points;
  }, 0);

  document.querySelector(".logros strong").innerText = userTaskDone.length + "/" + userTasks.length;
...
function cargarTareas() {
  ...
  const tareasEspacio = db.tasks.filter((t) => t.spaceId === espacio.id);

  const misTareas = tareasEspacio.filter((t) => t.assignedTo === usuario.id);
...
function renderCalendario() {
  ...
  container.innerHTML = Object.keys(tareasPorFecha)
    .map(
      (fecha) => `
...
window.crearTareasDemo = function () {
  ...
  const espacio = db.spaces.find((s) => s.id === db.session?.spaceId);
